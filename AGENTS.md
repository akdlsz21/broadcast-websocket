...
TypeScript sources live in `src/`, with the production entry in `src/index.ts` and the public class in `src/shared-ws.ts`. Leader ownership lives in `src/transport.ts`, leader election in `src/election.ts`, the BroadcastChannel bus in `src/bus.ts`, and the typed bus protocol in `src/protocol.ts`. Public types stay in `src/types.ts`, and compiled artifacts land in `dist/` after a build. HTML demos under `demo/` exercise multi-context scenarios, while `specs.md` captures design intent that should stay authoritative when changes are proposed.
